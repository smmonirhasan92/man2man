...
Executing: cat /var/www/man2man/backend/modules/wallet/withdrawal.controller.js
...
exports.requestWithdrawal = async (req, res) => {
    try {
        ...
        const result = await TransactionHelper.runTransaction(async (session) => {
            ...
            // To encourage P2P (which has 1.2% fee), we charge 3.5% here.
            const ADMIN_FEE_PERCENT = 0.035;
            const feeAmount = parsedAmount * ADMIN_FEE_PERCENT;
            const totalDeduction = parsedAmount + feeAmount;

            if (currentBalance < totalDeduction) {
                throw new Error(`Insufficient ${targetWallet} Balance. Note: An 3.5% platform fee applies. You need ${totalDeduction.toFixed(2)} NXS.`);
            }
            ...
            // Deduct total amount (Withdrawal + Fee)
            user.wallet[targetWallet] = (currentBalance - totalDeduction);
