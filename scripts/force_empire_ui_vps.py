...
file_path = '/var/www/man2man/frontend/components/referral/ReferralDashboard.js'
...
export default function ReferralDashboard() {
    ...
    const handleShareAction = async () => {
        try {
            // 1. Trigger Backend Share Tracking (with anti-spam)
            const res = await api.post('/referral/share');
            
            // 2. Record IP Handshake for auto-fill
    ...
    return (
        <div className="space-y-6 pb-20 animate-in fade-in duration-700">
            ...
            {/* 2. ELITE SHARE & HANDSHAKE SECTION */}
                ...
                <div className="relative space-y-12">
                    ...
                    <div className="relative pl-12">
                        ...
                        <div className="flex flex-col">
                            ...
                            {data.empire.shares >= 100 && !data.empire.isShareBonusClaimed && (
                                <span className="mt-2 text-[10px] text-indigo-400 font-black animate-pulse">PENDING IN HOLD BALANCE</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="relative pl-12">
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="mt-3 h-1.5 w-full max-w-[200px] bg-white/5 rounded-full overflow-hidden">
                                <div 
                                    className="h-full bg-emerald-500 transition-all duration-1000"
                                    style={{ width: `${data.empire.joinProgress}%` }}
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative pl-12 opacity-50">
                        <div className="absolute left-0 top-0 w-10 h-10 rounded-2xl border-2 border-dashed border-white/10 bg-[#0f172a] flex items-center justify-center">
                            <Gift className="w-5 h-5 text-slate-700" />
                        </div>
                        <div className="flex flex-col">
                            <h4 className="font-black text-sm uppercase text-slate-700">Grand Empire Reward</h4>
                            <p className="text-[10px] text-slate-800 font-bold uppercase tracking-widest mt-1">Complete both milestones to unlock Elite Badge</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-4 animate-in slide-in-from-bottom-2">
                        ...
                        {data.network.length > 0 ? data.network.map(member => (
                            <div key={member._id} className="bg-white/5 p-4 rounded-2xl flex items-center justify-between border border-white/5">
                                ...
                                <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg ${member.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                                    {member.status}
                                </span>
                            </div>
                        )) : (
...
with open(file_path, 'w') as f:
    f.write(content)
print("Successfully forced Empire 2.0 UI to VPS.")
