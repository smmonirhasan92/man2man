...
const deployFile = (localPath, remotePath) => {
    ...
    SSH(`> /tmp/deploy_admin_b64.txt`);
    
    const chunkSize = 4000;
    for (let i = 0; i < b64.length; i += chunkSize) {
        const chunk = b64.substring(i, i + chunkSize);
        SSH(`echo -n '${chunk}' >> /tmp/deploy_admin_b64.txt`);
    }
    
    SSH(`base64 -d /tmp/deploy_admin_b64.txt > ${remotePath}`);
...
try {
    ...
    console.log('✅ Admin Controller deployed successfully!');
} catch (e) {
