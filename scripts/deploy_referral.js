...
const deployFile = (localPath, remotePath) => {
    ...
    SSH(`> /tmp/deploy_ref_b64.txt`);
    
    const chunkSize = 4000;
    for (let i = 0; i < b64.length; i += chunkSize) {
        const chunk = b64.substring(i, i + chunkSize);
        SSH(`echo -n '${chunk}' >> /tmp/deploy_ref_b64.txt`);
    }
    
    SSH(`base64 -d /tmp/deploy_ref_b64.txt > ${remotePath}`);
    console.log(`✅ ${localPath} deployed.`);
};

try {
    deployFile('backend/modules/referral/ReferralController.js', '/var/www/man2man/backend/modules/referral/ReferralController.js');
    
    console.log('Restarting backend...');
    SSH(`cd /var/www/man2man && docker compose -f docker-compose.prod.yml restart backend`);
    
    console.log('✅ Referral Controller deployed successfully!');
} catch (e) {
