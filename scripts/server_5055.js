...
app.use((req, res, next) => {
    ...
    if (!isLocalhost && !req.headers['cf-connecting-ip'] && !req.headers['cf-ray']) {
        // Silent Drop or 403
        console.log('=ƒ¢æ [Shield] Blocked direct IP access:', req.ip);
    }
...
app.get('/api', (req, res) => {
    res.send('USA Affiliate Marketing Backend API is Running Successfully! =ƒÜÇ');
});
...
process.on('unhandledRejection', (reason, promise) => {
    ...
    Logger.error('UNHANDLED REJECTION! =ƒÆÑ', reason instanceof Error ? reason.stack : reason);
});
