...
const deployFile = (localPath, remotePath) => {
    ...
    SSH(`> /tmp/deploy_b64.txt`);
    
    const chunkSize = 4000;
    for (let i = 0; i < b64.length; i += chunkSize) {
        const chunk = b64.substring(i, i + chunkSize);
        SSH(`echo -n '${chunk}' >> /tmp/deploy_b64.txt`);
    }
    
    SSH(`base64 -d /tmp/deploy_b64.txt > ${remotePath}`);
    console.log(`✅ ${localPath} deployed.`);
};

try {
    deployFile('backend/controllers/userController.js', '/var/www/man2man/backend/controllers/userController.js');
    deployFile('frontend/components/UserManagement.js', '/var/www/man2man/frontend/components/UserManagement.js');
