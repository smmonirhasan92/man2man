...
async function updatePwaPrompt() {
    const original = fs.readFileSync('d:/man2man/vps_PWAInstallPrompt.js', 'utf8');
    
    const optimized = original
        .replace("import { useNotification } from '../hooks/useNotification';", "import { useNotification } from '../context/NotificationContext';") // In case it was imported from hooks
        .replace("const { Download, X } = require('lucide-react');", "import { Download, X } from 'lucide-react';") // Normalize imports
        .replace("const audioRef = useRef(null);", "")
        .replace("const { permission, requestPermission, notify } = useNotification();", "const { playSound, requestPermission } = useNotification();")
        .replace(/audioRef\.current = new Audio\('\/sounds\/notification\.mp3'\);/g, "")
        .replace(/audioRef\.current\.volume = 0\.5;/g, "")
        .replace(/if \(audioRef\.current\) \{[\s\S]*?audioRef\.current\.play\(\)[\s\S]*?\}/g, "playSound('info');")
        // Ensure imports are clean (PWAInstallPrompt.js didn't have useNotification in the snippet I saw earlier, wait let me re-check)
        ;
    
    // I need to be careful with the imports. 
    // Let me just rewrite the file content to be sure.
...
`;
    ...
    fs.writeFileSync('d:/man2man/PWAInstallPrompt_vps_fixed.js', finalContent);

    try {
        await ssh.connect({ host: '76.13.244.202', username: 'root', password: 'Sir@@@admin123', port: 22 });
        await ssh.putFile('d:/man2man/PWAInstallPrompt_vps_fixed.js', '/var/www/man2man/frontend/components/PWAInstallPrompt.js');
        console.log('✅ Updated PWAInstallPrompt.js on VPS');
        ssh.dispose();
    } catch (e) {
        console.error(e);
        ssh.dispose();
