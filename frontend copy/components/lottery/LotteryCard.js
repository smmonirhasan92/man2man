        ...
        <div className="bg-slate-800 p-4 rounded-xl border border-white/10 flex justify-between items-center hover:border-yellow-500/50 transition-colors group">
            <div>
                ...
                <div className="mt-2 flex items-center gap-3">
                    <p className="text-emerald-400 font-bold text-sm bg-emerald-500/10 px-2 py-1 rounded">Pool: ৳{lottery.prizePool}</p>
                    <p className="text-slate-400 text-xs">Tickets: {lottery.tickets?.length || 0}</p>
                </div>
            </div>
            <div className="text-right flex flex-col items-end gap-3">
                <span className="block text-2xl font-black text-white">৳{lottery.price}</span>
                <button
                ...
                >
                    <Ticket className="w-4 h-4" /> Buy Ticket
                </button>
            </div>
        </div>
    );
